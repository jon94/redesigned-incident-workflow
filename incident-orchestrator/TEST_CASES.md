...
## Test Case 5: Duplicate Workflow ID

**Objective:** Verify that starting a workflow with the same service name fails (one incident per service).

```bash
# 1. Start first incident
go run cmd/starter/main.go -cmd=start -service=duplicate-test

# 2. Try to start another with same service
go run cmd/starter/main.go -cmd=start -service=duplicate-test
```
